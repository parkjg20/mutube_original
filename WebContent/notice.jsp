				...
				<div class="container">
					<div class="row">
						<table class="table table-striped"
							...
							<tbody>
								...
								<tr>
									...
									<td class="title"><a href="noticeView.jsp?pn=<%=list.get(i).getPn()%>"><%=list.get(i).getTitle()%></a></td>
									<td class="uid"><%=list.get(i).getUid()%></td>
									<td class="date"><%=list.get(i).getPost_date()%></td>
								</tr>
								<%
									}
								%>
						...
						<%
							if(pageNumber != 1){
						%>
							<a href="notice.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-primary btn-arrow-left">Previous</a>
						<%
							} if(noticeDAO.nextPage(pageNumber + 1)){
						%>
							<a href="notice.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-primary btn-arrow-right">Next</a>
						<%
							}
						%>
