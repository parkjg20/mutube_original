...
<body>
	<%
		...
		String video = null;
		if (music.getVideo_link() != null) {
			video = music.getVideo_link();
		}
	%>
	...
	<div class="col-xs-9">
		<div class="container">
			<div class="row">
				...
				<table class="table"
					...
					<tbody>
						<tr>
							...
							<td colspan="4"><%=music.getMusic_title()%></td>
						</tr>
						<tr>
							<td>작성자</td>
							<td colspan="4"><%=music.getUid()%></td>
						</tr>
						<tr>
							<td>작성일자</td>
							<td colspan="4"><%=music.getDate()%></td>
						</tr>
						<tr>
							<td colspan="5">아티스트 정보 : <%=music.getName()%></td>
						</tr>
						<%
							if (video != null) {
						%>
						<tr>
							<td colspan="5"><iframe
									src="https://www.youtube.com/embed/<%=video%>" width="720"
									height="340" allowFullScreen></iframe></td>
						</tr>
						<%
							}
						%>
						...
						<tr>
							<td>내용</td>
							<td colspan="4" style="min-height: 200px; text-align: left;"><%=music.getContent()%></td>
						</tr>
						<tr>
						...
						<%
							...
							ArrayList<Mreply> list = musicDAO.getReply(pageNumber, pn);
							for (int i = 0; i < list.size(); i++) {
						%>
						<tr>
							<td style="border-right: 1px solid #dddddd;"><%=list.get(i).getUid()%></td>
							<td colspan="3" class="replyContent"><%=list.get(i).getContent()%></td>
							<td style="width: 20px; margin-top: 0 margin;"><a href="replyDeleteAction.jsp?cat=music&pn=<%=pn %>&dn=<%=list.get(i).getDn() %>"><button
				...
				<a href="music.jsp" class="btn btn-primary">목록</a>
				<%
					if (uid != null && uid.equals(music.getUid())) {
				%>
				<a href="musicUpdate.jsp?pn=<%=pn%>" class="btn btn-primary">수정</a>
				<a onclick="return confirm('정말로 삭제하시겠습니까?')"
					href="musicDeleteAction.jsp?pn=<%=pn%>" class="btn btn-primary">삭제</a>
				<%
					}
				%>
